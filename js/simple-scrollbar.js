;(function(root, factory) {
	...
	function ss(el) {
		...
		this.bar.setAttribute('class', 'ss-bar');

		this.handle = d.createElement('div');
		this.handle.setAttribute('class', 'ss-scroll');

		//this.handle = '<div class="ss-scroll">';
		...
		this.el = d.createElement('textarea');
		this.el.setAttribute('name', this.target.getAttribute('data-name'))
		this.el.setAttribute('placeholder', this.target.getAttribute('data-placeholder'))
		...
		//this.target.insertAdjacentHTML('beforeend', this.handle);
		//this.handle = this.target.lastChild;

		dragDealer(this.handle, this);
		this.moveBar();
		...
		this.unBind = function() {
			...
			this.target.removeChild(this.handle);
			this.handle = null; //make way for the garbage collector
		}
	}
	...
	ss.prototype = {
		moveBar: function(e) {
			...
			var right = isRtl ?
				(_this.target.clientWidth - _this.handle.clientWidth + 18) :
				(_this.target.clientWidth - _this.handle.clientWidth) * -1;

			raf(function() {
				// Hide scrollbar if no scrolling is possible
				if(_this.scrollRatio >= 1) {
					_this.handle.classList.add('ss-hidden')
				} else {
					_this.handle.classList.remove('ss-hidden')
					_this.handle.style.cssText = 'height:' + Math.max(_this.scrollRatio * 100, 10) + '%; top:' + ((_this.el.scrollTop / totalHeight ) * 100) + '%;';
				}
			});
		}
	}
